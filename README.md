...
for it. Each browser reads the whole message aloud as one utterance, using
the Web Speech API, with each station assigned a distinct, consistent voice
for the session (picked from the browser's available voices by hashing the
station label) so listeners can tell speakers apart by ear. Speaking a
message in one utterance, rather than one per word, keeps the engine's own
sentence prosody instead of each word coming out flat and isolated. Speech
isn't synced to the on-screen word reveal, which still runs at its own pace
independently and sits collapsed under a "Show words on screen" fold instead
